...
// Remove the corrupted part (the last part with backslashes)
const corruptedIndex = content.indexOf('export const buildCustomerBatchReportCarouselFlexMessage');
if (corruptedIndex !== -1) {
    content = content.substring(0, corruptedIndex);
}
...
fs.writeFileSync(path, content + func);
console.log('Fixed lineMessaging.ts');
